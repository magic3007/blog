...
不失去一般性，对于一个函数
$$
y=f(x),\ x \in \R^n, y \in \R^m
$$
它的雅可比矩阵是：
$$
J_f = \frac{\partial f}{\partial x} \in \R^{m \times n}
$$
**JVP：Jacobian-Vector Product（正向微分）**的定义是，给定一个向量$v \in \R^n$, JVP计算
$$
J_f(x)\cdot v
$$
...
**VJP：Vector-Jacobian Product（反向微分）**则相反，其定义是，给定一个向量$u \in \R^m$, JVP计算：
$$
