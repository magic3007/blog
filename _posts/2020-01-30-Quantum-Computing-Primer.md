...
不妨设这组正交归一基为$$\{ \vert \psi_i \rangle \}$$, 则$$O$$在该组基底下的矩阵表示为$$diag\{\lambda_0, \lambda_2, \dots, \lambda_{n-1}\}(\lambda_i \in \R)$$, 显然$$\{\lambda_i\}$$和$$\{\vert\psi_i\rangle\}$$恰为本征值和本征态, 即$$O\vert\psi_i \rangle = \lambda_i \vert \psi_i \rangle$$.
